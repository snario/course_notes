...
\begin{document}
  ...
  \section{Continuous Observables in Quantum Mechanics}
    ...
    \subsubsection{Zero Point Energy}
      ...
      \[ \E{\hat{x}} = \bra{E_1}\hat{x}\ket{E_1} = \int_{-\infty}^{\infty} dx \Psi_1^*(x)x\Psi_1(x) = \f{2}{L} \int_0^L x\sin^2 \left( \f{\pi x}{L} \right) \]

      and finally the probability,

      \[ prob\left(0<x<\f{L}{3}\right) = \int_0^{\f{L}{3}}\Psi_1^*(x)\Psi_1(x) = \f{2}{L} \int_0^{\f{L}{3}} \sin^2 \left( \f{\pi x}{L} dx = 0.195 \right) \]
    ...
    \subsection{Harmonic Oscillator Potential}
      ...
      Since $E > V(x)$, then $k$ is real and so we expect
